...
class Attendance extends CI_Controller {
    ...
    $data = $this->input->post();
           ...
           // foreach($data['emp_id'] as $emp_id){
            ...
            //  if($this->attendance->employee_attendence_exist($emp_id,$data['attendance_date'])==false){
    			...
    			$task = array(':leave_type' => $data['leave-type'],
            ...
            ':end_date' => $endDate);
                  ...
                  'days' => $data['leave_days']
    				...
    				print_r($record); exit();
                  $res = $this->leave->save_data($record);
                  if($res==true){
            ...
            redirect(base_url().'admin/attendance/leave_type');   
          } 
    /*
  if(isset($_POST["action"])):
        ...
        switch($_POST["action"]):
            ...
            case "addData":
                //if($authority == 1):
                    if(!empty($_POST["leave-type"])):
                        $data_info = array(
                                                    "unit_type"          =>      htmlspecialchars($_POST["unit-type"], ENT_QUOTES)
                                        );
                        /*  $data_log = array(
                                            array(
                                                    "action"                =>      "added",
                     ...
                     /*   $tableData["unit_type_info"] = json_encode($data_info);
                        $tableData["unit_type_log"] = json_encode($data_log);
...
public function fetchAllLeaveData() {
			$leave= $this->leave->get_leave();
    ?>
     <form action="<?php echo base_url(); ?>admin/Attendance/add_emp_attendence" class="form-horizontal attendance_add_form" id="form_data" method="post" enctype="multipart/form-data" onsubmit="return dateVerification()">
         ...
         <tr>
            ...
            <td>
               <center style="display: flex;">
                  <input class="form-control btn btn-info" type="button" name="edit<?php echo $row['id']?>" value="Edit" style="width: 93px; margin:0 10px">
                 <input class="form-control btn btn-danger" type="button" name="delete<?php echo $row['id']?>" value="Delete" style="width: 93px;">
               </center>
            </td>
             </tr>
         <?php
  ...
  function approve_employeeLoanData($cid,$approve)
  {

    $data=array();
    $this->Payroll_model->approve_employeeLoanData($cid,$approve);
    $this->session->set_userdata('msg',"<div style='color:green;'>Loan Approved successfully</div>");
    $this->session->set_userdata('class','green_msg');

    redirect(base_url().'admin/attendance/loan_approval');
  }

  
  function reject_employeeLeaveData($cid,$reject)
  {


    $data=array();
    $this->Payroll_model->reject_employeeLeaveData($cid,$reject);
    $this->session->set_userdata('msg',"<div style='color:green;'>Leave Rejected successfully</div>");
    $this->session->set_userdata('class','green_msg');

    redirect(base_url().'admin/attendance/leave_management');
  }
  ...
  public function loan_approval()
  {
      ...
      $this->data['page'] = 'Loan Approval'; 
   

      $this->load->view('admin/include/header',$this->data);
     $this->load->view('admin/include/sidebar',$this->data);   
     $this->load->view('admin/attendance/loan_approval',$this->data);
  }
  
    
    
    public function fetchAllLoanApprovalData()
  {
          
      $loanApprovalData=$this->Payroll_model->getLoanApprovalData();
    ...
    ?>
      ...
      <th class="text-nowrap">Loan Type</th>
      <th class="text-nowrap">Loan Date</th>
    ...
    <?php
    $sno = 1;
    if(!empty($loanApprovalData))
    {
      foreach($loanApprovalData as $row)
      {
    ...
    ?>  
        ...
        <tr>
               ...
               <button type="button" class="btn btn-sm btn-warning btn-editleads" onclick="editData(this)" data-toggle="modal" data-id="<?=$row["loan_id"]; ?>" data-target="#editData">Pay EMI</button> 
               <button type="button" class="btn btn-sm btn-warning btn-editleads" onclick="editData1(this)" data-toggle="modal" data-id="<?=$row["emp_id"]; ?>" data-target="#editData1">Paid EMI Details</button> 
  ...
  <?php }
		...
		public function editData()
		{
			...
			$emidata = $this->Payroll_model->getDataById($id);
          //echo "<pre>";
         // print_r($emidata['loan_id']);
           $lastremaing_amt = $this->Payroll_model->getremngamtbyloanId($emidata['loan_id']);
         //echo "<pre>";
         // print_r($lastremaing_amt);
            ...
            <div class='row'>
                   ...
                   <div class='row'>
                        <div class='col-md-4'>
                            <div id='response' class='form-group'>
                                <label class='control-label'><strong>Loan Total Amount:</strong></label>
                              <input class='form-control form-control-sm' name='loan_amount' type='text'  id='' required='' value='".$emidata['loan_amount']."' readonly>
                             <input class='form-control form-control-sm' type='hidden' name='loan_id' id='id' required='true' value='".$emidata['loan_id']."'>

                            </div>
                            </div>";
                            
                            if($lastremaing_amt[0]['lastAmount'] == '')
                            {
                               $output .="<div class='col-md-4'>
                            <div id='response' class='form-group'>
                                <label class='control-label'><strong>Dues:</strong></label>
                              <input class='form-control form-control-sm' name='' type='text' onkeyup='getAmount()' id='last_payment_amount' required='' value='".$emidata['loan_amount']."' readonly>
                            ...
                            <div id='response' class='form-group'>
                                <label class='control-label'><strong>Dues:</strong></label>
                              <input class='form-control form-control-sm' name='' type='text' onkeyup='getAmount()' id='last_payment_amount' required='' value='".$lastremaing_amt[0]['lastAmount']."' readonly>
                            ...
                            $output .="<div class='col-md-4'>
                            <div id='response' class='form-group'>
                                <label class='control-label'><strong>Loan EMI:</strong></label>
                             <input class='form-control form-control-sm' type='text' name='loan_emi' onkeyup='getAmount()' value='' id='loan_emi' required='true' value='' autocomplete='off'>
                            </div>
                            </div>
                             ...
                             <div class='col-md-4'>
                            <div id='response' class='form-group'>
                                <label class='control-label'><strong>Remaining Amount:</strong></label>
                             <input class='form-control form-control-sm' type='text' name='remaining_amount'  onkeyup='getAmount()' id='remaining_amount' required='true' value='' >
                            </div>
                            </div>
                             ...
                             <div class='col-md-4'>
                            <div id='response' class='form-group'>
                                <label class='control-label'><strong>Loan Pay Date:</strong></label>
                             <input class='form-control form-control-sm' type='date' name='loan_pay_date' id='id' required='true' value='' >
                            </div>
                            </div>
				...
				<button type='submit' class='btn btn-primary' id='save_data_changes'>Pay</button>
				</div>";
			echo $output;
		}
      	...
      	function update_data()
		{
            ...
            //$update_data = $this->Services_model->getDataById($postdata['id']);			
            ...
            $data1 = array(
							...
							'loan_pay_date' => $this->input->post('loan_pay_date'),
               ...
               //  echo "<pre>";
				...
				$this->Payroll_model->updateLoan($this->input->post('loan_id'),$data);
					
                        if($this->Payroll_model->inertLoanData($data1))
                        {
						$data['status']=true;
  ...
  {
     $this->data['page'] = 'Leave Management'; 
    // $this->data['department']= $this->employee_model->get_data_array('tbl_department');
     ...
     $this->load->view('admin/include/header',$this->data);
     $this->load->view('admin/include/sidebar',$this->data);   
     $this->load->view('admin/attendance/leave_management',$this->data);
  }
   ...
   // $this->data['store']=$this->Store->getAllStores();
	...
	public function attendance_report()
	{
	    ...
	    $data['employees'] = $this->employee_model->get_all_employee('tbl_manage_employee');
    ...
    public function add_emp_attendence()
    {
        ...
        foreach ($data['emp_id'] as $emp_id) {
            ...
            if ($this->attendance->employee_attendence_exist($emp_id, $data['attendance_date']) == false) {
                $record = array(
                    ...
                    'overtime_amount'=>$data['overtime_amount'.$emp_id],
                    'incentive'=>$data['incentive'.$emp_id],
                    'disincentive'=>$data['disincentive'.$emp_id],
                    'conveyance'=>$data['conveyance'.$emp_id]
                );
        ...
        redirect(base_url() . 'admin/attendance/attendance_view');
    }
...
<?php
    ...
    public function show_attendance_data()
      ...
      public function show_attendance_report_view()
            {   
                 ...
                 $data=$this->attendance->get_leave_report_data($_POST['employee_id'],$_POST['start_date'],$_POST['end_date'],$_POST['status']);
    ...
    public function fetchAllEmployeeLeaveData()
  {
          
      $employeeleaveData=$this->Payroll_model->getEmployeeLeaveData();
    ...
    ?>
      ...
      <th class="text-nowrap">Leave Date</th>
      <th class="text-nowrap">No Of Days</th>
    ...
    ?>  
        ...
        <tr>
                      <td><?php echo $sno++; ?></td>
                      <td><?php echo $row['emp_name']; ?></td>
                      <td><?php echo $row['emp_leave_type']; ?></td>
                      <td><?php echo $row['emp_start_date']; ?></td>
                      <td><?php echo $row['emp_no_of_days']; ?></td>
                      <td><?php echo $row['emp_desc_reagon']; ?></td>
                    
                      <td>
                      
                      <?php  if($row['status'] == '') { ?>
                      <a rel="tooltip" title="Remove" class="btn btn-link btn-sm btn-primary table-action remove" onClick="return confirm('Are you sure you want to approve?')" href="approve_employeeLeaveData/<?php echo $row['emp_id'];?>/Approved"> Approve </a>

                      <a rel="tooltip" title="Remove" class="btn btn-link btn-sm btn-danger table-action remove" onClick="return confirm('Are you sure you want to reject?')" href="reject_employeeLeaveData/<?php echo $row['emp_id'];?>/Rejected">
                      Reject       
                       </a>
                      ...
                      <a rel="tooltip" title="Remove" class="btn btn-link btn-sm btn-success table-action remove" onClick="" href="" style="
                  background-color: #29aa29; border-color: #29aa29" >
                      ...
                      <button type="button" class="btn btn-primary" disabled> Approve</button>
                        ...
                        <a rel="tooltip" title="Remove" class="btn btn-link btn-sm btn-danger table-action remove" onClick="" href="" > Rejected </a>    
  ...
  function approve_employeeLeaveData($cid,$approve)
  {


    $data=array();
    $this->Payroll_model->approve_employeeLeaveData($cid,$approve);
    $this->session->set_userdata('msg',"<div style='color:green;'>Leave Approved successfully</div>");
    $this->session->set_userdata('class','green_msg');
...
// fetch data from db
    ...
    {
        $department = $this->employee_model->get_data_array('tbl_manage_department');
        $employees = $this->employee_model->getAllEmpByStoreID();
        //echo "<pre>";
        //print_r($employees);
    ?>
        <form action="<?php echo base_url(); ?>admin/Attendance/add_emp_attendence" class="form-horizontal attendance_add_form" id="form_data" method="post" enctype="multipart/form-data" onsubmit="return dateVerification()">
            ...
            <table id="data-table-buttons" class="table table-striped table-bordered table-td-valign-middle">
               ...
               -->
                        ...
                        <th class="text-nowrap">Extra Income</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    ...
                    if (!empty($employees)) {
                        foreach ($employees as $row) {
                            ...
                            $department = $this->dm->get_department_by_id($manage_employee_info->department);

                            // $designation = $this->designation->get_designation_by_id($manage_employee_info->designation);
                    ?>
                            <tr>
                                <td>
                                    <center><input type="checkbox" class='item largerCheckbox' name="emp_id[]" value="<?php echo  $row['manage_employee_id']; ?>"></center>
                                </td>
                                <td><?php echo $manage_employee_info->firstName; ?> <?php echo  $manage_employee_info->lastName; ?></td>
                                <td><?php echo $manage_employee_info->department ?></td>
                                <!--<td><?php //echo $designation['designation'] 
                                        ?></td>-->
                                <td>
                                    <center>
                                        <input class="form-control" type="hidden" name="emp_id<?php echo $row['manage_employee_id'] ?>">

                                        <input class="form-control" type="text" name="comment<?php echo $row['manage_employee_id'] ?>">
                                    </center>
                                </td>
                                <td>
                                    <center>
                                        <select class="form-control" name="status<?php echo $row['manage_employee_id'] ?>" id="status" onchange="statusselect(this.value,'<?php echo $row['manage_employee_id'] ?>');" value="" style="width:80%;">
                                            <option value="Present">Present</option>
                                            <option value="Absent">Absent</option>
                                            <option value="On Leave">On Leave</option>
                                    ...
                                    <center>
                                       ...
                                       $lastdate = date('t',strtotime($curr_date)); ?>

                                          <input  type="text" id="overtime<?php echo $row['manage_employee_id'] ?>" onkeyup="calovertime(this.value,<?php echo $row['manage_employee_id'] ?>);" name="overtime<?php echo $row['manage_employee_id'] ?>" style="display:none" placeholder="Overtime In Hour" class="form-control get-overtime"><br>
                                          <input  type="hidden" id="empSalary<?php echo $row['manage_employee_id'] ?>" onkeyup="calovertime(this.value,<?php echo $row['manage_employee_id'] ?>);" name="" value="<?php echo $manage_employee_info->totalSalary ?>" style="display:none" class="form-control"><br>
                                          <input  type="hidden" id="attlastDate<?php echo $row['manage_employee_id'] ?>" onkeyup="calovertime(this.value,<?php echo $row['manage_employee_id'] ?>);" name="" value="<?php echo $lastdate ?>" style="display:none" class="form-control"><br>
                                        <input class="form-control" type="text" id="overtime_amount<?php echo $row['manage_employee_id'] ?>" name="overtime_amount<?php echo $row['manage_employee_id'] ?>" style="display:none" placeholder="Overtime Amount" readonly>
                                    </center>

                                <td>

                                    <center>
                                        <input class="form-control" type="text" id="incentive" name="incentive<?php echo $row['manage_employee_id'] ?>" placeholder="Incentive">
                                    </center><br>

                                    <center>
                                        <input class="form-control" type="text" id="disincentive" name="disincentive<?php echo $row['manage_employee_id'] ?>" placeholder="Disincentive">
                                    </center><br>

                                    <center>
                                        <input class="form-control" type="text" id="conveyance" name="conveyance<?php echo $row['manage_employee_id'] ?>" placeholder="Conveyance">
                                    </center>
                                </td>
                                </td>
                            </tr>
