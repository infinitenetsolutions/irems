	   ...
	   public function get_all_attendance($tbl)
		{
			...
			$this->db->select ('tbl_manage_employee.*,tbl_attendance.*'); 
			$this->db->from($tbl);
			$this->db->join('tbl_manage_employee', 'tbl_attendance.emp_id = tbl_manage_employee.manage_employee_id');
			$query = $this->db->get();
...
public function get_attendance_data($attendance_date)
		 ...
		 public function get_leave_report_data($empid,$start_date,$end_date,$status)
		 {
           
		  $this->db->select("*");
		  $this->db->from('tbl_attendance');
		  $this->db->join('tbl_manage_employee', 'tbl_attendance.emp_id = tbl_manage_employee.manage_employee_id', 'LEFT');
		  $this->db->where('tbl_attendance.status',$status);
		  if ($empid != NULL) {
			/*-- =->where('tbl_attendance.emp_id',$get_leave_report_data); */
		  } 
		  
		  	$this->db->where('tbl_attendance.attendance_date BETWEEN "'.$start_date.'" and "'.$end_date.'"');
		   	return $this->db->get()->result_array();
		 }
