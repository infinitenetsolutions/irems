<?php 
    ...
    $authority = 1;
    $manageCompanyStoreDir = "../../../assets/admin/building/";
    $manageCompanyDir = "assets/admin/building/";
    if(isset($_POST["action"])):
        ...
        switch($_POST["action"]):
            ...
            case "fetchData":
                ?>
                    <form id="selectForm" method="POST" enctype="multipart/form-data" action="application/controller/admin/building.php">
                        <input type="hidden" id="nameOfATable" name="nameOfATable" value="tbl_building">
                                    ...
                                    <?php 
                                        ...
                                        if($getData != 0):
                                            ...
                                            foreach($getData as $rows):
                                                ...
                                                $building_log = json_decode($rows["building_log"]);
                                    ?>
                                                    <tr>
                                                        <td class="text-center">
                                                            <div class="icheck-navy d-inline">
                                                                <input type="checkbox" id="checkbox-<?= $rows["building_id"] ?>" name="checkbox-select[]" value="<?= $rows["building_id"] ?>" class="check-table">
                                                                <label for="checkbox-<?= $rows["building_id"] ?>">
                                                                </label>
                                                            </div>
                                                        </td>
                                                        <td><?= $sno ?>.</td>
                                                        ...
                                                        <td class="text-center">
                                                            <button type="button" id="information-button-<?= $rows["building_id"] ?>" class="information-button btn btn-xs btn-danger mt-1 mb-1" title="Informations">
                                                                <i class="fa fa-scroll fa-sm"></i>
                                                            </button>
                                                            <button type="button" id="see-button-<?= $rows["building_id"] ?>" class="see-button btn btn-xs btn-info mt-1 mb-1" title="See">
                                                                <i class="fa fa-eye fa-sm"></i>
                                                            </button>
                                                            <button type="button" id="edit-button-<?= $rows["building_id"] ?>" class="edit-button btn btn-xs btn-warning mt-1 mb-1" title="Edit/Update">
                                                                <i class="fa fa-edit fa-sm"></i>
                                                            </button>
                                                            <button type="button" id="delete-button-<?= $rows["building_id"] ?>" class="delete-button btn btn-xs btn-danger mt-1 mb-1" title="Delete">
                                                    ...
                                                    <script>
                                                        ...
                                                        $("#information-button-<?= $rows["building_id"] ?>").click(function () {
                                                            ...
                                                            var formData = {"action":"fetchInformation","id":"<?= $rows["building_id"] ?>"};
                                                            $.ajax({
                                                                url: 'application/view/admin/building.php',
                                                                type: 'POST',
                                                                data: formData,
                                                        ...
                                                        // See Section Start ---------------------------------------------------------------
                                                        $("#see-button-<?= $rows["building_id"] ?>").click(function () {
                                                            ...
                                                            var formData = {"action":"fetchSee","id":"<?= $rows["building_id"] ?>"};
                                                            $.ajax({
                                                                url: 'application/view/admin/building.php',
                                                                type: 'POST',
                                                                data: formData,
                                                        ...
                                                        $("#delete-button-<?= $rows["building_id"] ?>").click(function () {
                                                            ...
                                                            var formData = {"action":"fetchDelete","id":"<?= $rows["building_id"] ?>"};
                                                            $.ajax({
                                                                url: 'application/view/admin/building.php',
                                                                type: 'POST',
                                                                data: formData,
            ...
            case "fetchInformation":
                if($authority == 1):
                    if(isset($_POST["id"]) && !empty($_POST["id"])):
                        $databaseObj->select("tbl_building");
                        $databaseObj->where("`status` = '".$auth->visible()."' && `building_id` = '".$_POST["id"]."'");
                        $databaseObj->order_by("`building_id` DESC");
                        $getData = $databaseObj->get();
                        //Checking If Data Is Available
                        if($getData != 0):
                            foreach($getData as $rows):
                                ...
                                ?>
                                    <div class="row">
                                        <?php
                                            $sno = 1;
                                            foreach($building_log as $building_log_info):
                                            ?>
                                                <div class="col-md-12">
                                                    <div class="card">
                                                        <div class="card-header d-flex p-0">
                                                            <ul class="nav nav-pills ml-auto p-2">
                                                                <li class="nav-item"><a class="nav-link" href="#tab_4_<?= $sno ?>" data-toggle="tab"><?= ucfirst($building_log_info->action) ?> By</a></li>
                                                        ...
                                                        <div class="card-body">
                                                            <div class="tab-content">
                                                                <div class="tab-pane" id="tab_4_<?= $sno ?>">
                                                                    <h5><i class="icon fas fa-user-alt"></i> <?= ucfirst($building_log_info->action) ?> By - 
                                                                    <?php
                                                                        if($building_log_info->by == $auth->admin_id):
                                                                            echo "You";
                                                                        else:
                                                                            $databaseObj->select("tbl_admin");
                                                                            $databaseObj->where("`status` = '".$auth->visible()."' && `admin_id` = '".$building_log_info->by."'");
                                                                            $getData = $databaseObj->get();
                                                                            //Checking If Data Is Available
                                                                            if($getData != 0):
                                                                                foreach($getData as $rows):
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" id="editBuilding" name="editBuilding" placeholder="Block" value="<?= $building_info->building ?>">
                                            </div>
                                        </div>
            ...
            case "fetchDelete":
                if($authority == 1):
                    if(isset($_POST["id"]) && !empty($_POST["id"])):
                        ...
                        <input type="hidden" id="tableName" name="tableName" value="tbl_building" />
                        <?php
                    else:
                        ?>
                        <div class="alert alert-danger alert-dismissible">
