<?php 
    ...
    $authority = 1;
    $manageCompanyStoreDir = "../../../assets/admin/phase/";
    $manageCompanyDir = "assets/admin/phase/";
    if(isset($_POST["action"])):
        ...
        switch($_POST["action"]):
            ...
            case "fetchData":
                ?>
                    <form id="selectForm" method="POST" enctype="multipart/form-data" action="application/controller/admin/phase.php">
                        <input type="hidden" id="nameOfATable" name="nameOfATable" value="tbl_phase">
                                    ...
                                    <?php 
                                        ...
                                        if($getData != 0):
                                            ...
                                            foreach($getData as $rows):
                                                ...
                                                $phase_log = json_decode($rows["phase_log"]);
                                    ?>
                                                    <tr>
                                                        <td class="text-center">
                                                            <div class="icheck-navy d-inline">
                                                                <input type="checkbox" id="checkbox-<?= $rows["phase_id"] ?>" name="checkbox-select[]" value="<?= $rows["phase_id"] ?>" class="check-table">
                                                                <label for="checkbox-<?= $rows["phase_id"] ?>">
                                                                </label>
                                                            </div>
                                                        </td>
                                                        <td><?= $sno ?>.</td>
                                                        <td><?= $phase_info->phase ?></td>
                                                        <td class="text-center">
                                                            <button type="button" id="information-button-<?= $rows["phase_id"] ?>" class="information-button btn btn-xs btn-danger mt-1 mb-1" title="Informations">
                                                                <i class="fa fa-scroll fa-sm"></i>
                                                            </button>
                                                            <button type="button" id="see-button-<?= $rows["phase_id"] ?>" class="see-button btn btn-xs btn-info mt-1 mb-1" title="See">
                                                                <i class="fa fa-eye fa-sm"></i>
                                                            </button>
                                                            <button type="button" id="edit-button-<?= $rows["phase_id"] ?>" class="edit-button btn btn-xs btn-warning mt-1 mb-1" title="Edit/Update">
                                                                <i class="fa fa-edit fa-sm"></i>
                                                            </button>
                                                            <button type="button" id="delete-button-<?= $rows["phase_id"] ?>" class="delete-button btn btn-xs btn-danger mt-1 mb-1" title="Delete">
                                                    ...
                                                    <script>
                                                        ...
                                                        $("#information-button-<?= $rows["phase_id"] ?>").click(function () {
                                                            ...
                                                            var formData = {"action":"fetchInformation","id":"<?= $rows["phase_id"] ?>"};
                                                            $.ajax({
                                                                url: 'application/view/admin/phase.php',
                                                                type: 'POST',
                                                                data: formData,
                                                        ...
                                                        // See Section Start ---------------------------------------------------------------
                                                        $("#see-button-<?= $rows["phase_id"] ?>").click(function () {
                                                            ...
                                                            var formData = {"action":"fetchSee","id":"<?= $rows["phase_id"] ?>"};
                                                            $.ajax({
                                                                url: 'application/view/admin/phase.php',
                                                                type: 'POST',
                                                                data: formData,
                                                        ...
                                                        $("#delete-button-<?= $rows["phase_id"] ?>").click(function () {
                                                            ...
                                                            var formData = {"action":"fetchDelete","id":"<?= $rows["phase_id"] ?>"};
                                                            $.ajax({
                                                                url: 'application/view/admin/phase.php',
                                                                type: 'POST',
                                                                data: formData,
            ...
            case "fetchInformation":
                if($authority == 1):
                    if(isset($_POST["id"]) && !empty($_POST["id"])):
                        $databaseObj->select("tbl_phase");
                        $databaseObj->where("`status` = '".$auth->visible()."' && `phase_id` = '".$_POST["id"]."'");
                        $databaseObj->order_by("`phase_id` DESC");
                        $getData = $databaseObj->get();
                        //Checking If Data Is Available
                        if($getData != 0):
                            foreach($getData as $rows):
                                ...
                                ?>
                                    <div class="row">
                                        <?php
                                            $sno = 1;
                                            foreach($phase_log as $phase_log_info):
                                            ?>
                                                <div class="col-md-12">
                                                    <div class="card">
                                                        <div class="card-header d-flex p-0">
                                                            <ul class="nav nav-pills ml-auto p-2">
                                                                <li class="nav-item"><a class="nav-link" href="#tab_4_<?= $sno ?>" data-toggle="tab"><?= ucfirst($phase_log_info->action) ?> By</a></li>
                                                        ...
                                                        <div class="card-body">
                                                            <div class="tab-content">
                                                                <div class="tab-pane" id="tab_4_<?= $sno ?>">
                                                                    <h5><i class="icon fas fa-user-alt"></i> <?= ucfirst($phase_log_info->action) ?> By - 
                                                                    <?php
                                                                        if($phase_log_info->by == $auth->admin_id):
                                                                            echo "You";
                                                                        else:
                                                                            $databaseObj->select("tbl_admin");
                                                                            $databaseObj->where("`status` = '".$auth->visible()."' && `admin_id` = '".$phase_log_info->by."'");
                                                                            $getData = $databaseObj->get();
                                                                            //Checking If Data Is Available
                                                                            if($getData != 0):
                                                                                foreach($getData as $rows):
                                                                ...
                                                                <div class="tab-pane active" id="tab_1_<?= $sno ?>">
                                                                    ...
                                                                    <?= date("l, M d, Y", strtotime($phase_log_info->date)) ?> At <?= $phase_log_info->at ?>
                                                                </div>
                                                                ...
                                                                <div class="tab-pane" id="tab_3_<?= $sno ?>">
                                                                    ...
                                                                    <?php
                                                                        $latLangArray = explode(",", $phase_log_info->location);
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" id="editPhase" name="editPhase" placeholder="Phase" value="<?= $phase_info->phase ?>">
                                            </div>
                                        </div>
            ...
            case "fetchDelete":
                if($authority == 1):
                    if(isset($_POST["id"]) && !empty($_POST["id"])):
                        ...
                        <input type="hidden" id="tableName" name="tableName" value="tbl_phase" />
                        <?php
                    else:
                        ?>
                        <div class="alert alert-danger alert-dismissible">
