<?php 
    ...
    if(isset($_POST["action"])):
        ...
        switch($_POST["action"]):
            ...
            case "addData":
                if($authority == 1):
                    if(!empty($_POST["clientName"] && $_POST["email"] && $_POST["priority"])):
                        if(!empty($_FILES["attachment"]["name"])):
                            if(move_uploaded_file($_FILES["attachment"]["tmp_name"] , $manageTicketDir.$randSix."_".$_FILES["attachment"]["name"])):
                                $attachment = $randSix."_".$_FILES["attachment"]["name"];
                            else:
                                $attachment = "default";
                            endif;
                        else:
                      ...
                      //$ticket_no = random_string('numeric',8);
                        ...
                        $complain_info = array(
                                                ...
                                                "attachment"            =>      htmlspecialchars($attachment, ENT_QUOTES),
                                                "status"                =>      "opened",
                                                "scope"                 =>       "",
                                                "emp_name" =>                  ""
                        ...
                        $tableData["complain_info"] = json_encode($complain_info);
                        $tableData["complain_log"] = json_encode($complain_log);
                        $tableData["status"] = $auth->visible();
                        $check = $databaseObj->insert("tbl_complaints", $tableData);
                        if($check == 1):
                    ...
                    if(!empty($_POST["nameOfATable"])):
                        if(count($_POST["checkbox-select"]) > 0):
                            ...
                            $objPHPExcel->getProperties()->setCreator("IREMS")
                                                         ...
                                                         ->setTitle("Complain Management")
                                                         ->setSubject("Complain Management")
                                                         ->setDescription("Complain Management $fileName.")
                            ...
                            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B2', "Complain Management");
                            cellColor('B2', '001F3F');
                                ...
                                if($getData != 0):
                                    foreach($getData as $rows):
                                        ...
                                        $objPHPExcel->setActiveSheetIndex(0)
                                            ...
                                            ->setCellValue('D'.$inc, $complain_info->email)
                                            ->setCellValue('E'.$inc, $complain_info->priority)
                                            ->setCellValue('F'.$inc, $complain_info->subject)
                                            ->setCellValue('G'.$inc, $complain_info->content)
                                            ->setCellValue('H'.$inc, $complain_info->status);
