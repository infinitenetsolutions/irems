<?php 
    ...
    $authority = 1;
    $propertyTypeDir = "../../../assets/admin/property-type/";
            ...
            case "addData":
                if($authority == 1):
                    if(!empty($_POST["propertyType"])):
                        $data_info = array(
                                                    "propertyType"          =>      htmlspecialchars($_POST["propertyType"], ENT_QUOTES)
                                        );
                        $data_log = array(
                                            array(
                                                    "action"                =>      "added",
                        ...
                        $tableData["property_type_info"] = json_encode($data_info);
                        $tableData["property_type_log"] = json_encode($data_log);
                        $tableData["status"] = $auth->visible();
                        $check = $databaseObj->insert("tbl_property_type", $tableData);
            ...
            case "importData":
                if($authority == 1):
                    if(!empty($_FILES["importedExcel"]["name"])):
                        if(move_uploaded_file($_FILES["importedExcel"]["tmp_name"] , $propertyTypeDir.$randSix."_".$_FILES["importedExcel"]["name"])):
                            $fileName = $randSix."_".$_FILES["importedExcel"]["name"];
                            $file_type	= PHPExcel_IOFactory::identify($propertyTypeDir.$fileName);
                            $objReader	= PHPExcel_IOFactory::createReader($file_type);
                            $objPHPExcel = $objReader->load($propertyTypeDir.$fileName);
                            $sheet_data	= $objPHPExcel->getActiveSheet()->toArray(null,true,true,true);
                            $sno = 0;
                            foreach ($sheet_data as $row):   
                                if(!empty($row["A"])):
                                    $data_info = array(
                                                            "propertyType"          =>      htmlspecialchars($row["A"], ENT_QUOTES)
                                                    );
                                    $data_log = array(
                                                        array(
                                                                "action"                =>      "imported",
                                    ...
                                    $check = $databaseObj->insert("tbl_property_type", $tableData);
                                    if($check == 1):
                                        unset($tableData["property_type_info"]); 
                                        unset($tableData["property_type_log"]); 
                            ...
                            endforeach;
                            if($sno > 0):
                                unlink($propertyTypeDir.$fileName);
                                $data["response"] = "success";
                                $data["responseType"] = "success";
            ...
            case "editData":
                if($authority == 1):
                    if(!empty($_POST["editTableId"])):
                        ...
                        $databaseObj->where("`status` = '".$auth->visible()."' && `property_type_id` = '".$_POST["editTableId"]."'");
                        $getData = $databaseObj->get();
                        //Checking If Data Is Available
                        if($getData != 0):
                            $sno = 1;
                            foreach($getData as $rows):
                                $data_info = json_decode($rows["property_type_info"]);
                                $data_log = json_decode($rows["property_type_log"]);
                            endforeach;
                            if(!empty($_POST["editPropertyType"])):
                                $edit_info = array(
                                                    "propertyType"          =>      htmlspecialchars($_POST["editPropertyType"], ENT_QUOTES)
                                                );
                                ...
                                $tableData["property_type_info"] = json_encode($edit_info);
                                $tableData["property_type_log"] = json_encode($data_log);
                                $tableData["status"] = $auth->visible();
                                $check = $databaseObj->update("tbl_property_type", $tableData, "`property_type_id` = '".$_POST["editTableId"]."'");
                                if($check == 1):
                                    $data["response"] = "success";
                                    $data["responseType"] = "success";
            ...
            case "deleteData":
                if($authority == 1):
                    if(!empty($_POST["tableName"] && $_POST["tableId"])):
                        $databaseObj->select("tbl_property_type");
                        $databaseObj->where("`status` = '".$auth->visible()."' && `property_type_id` = '".$_POST["tableId"]."'");
                        $getData = $databaseObj->get();
                        //Checking If Data Is Available
                        if($getData != 0):
                            $sno = 1;
                        ...
                        $check = $databaseObj->update($_POST["tableName"], $tableData, "`property_type_id` = '".$_POST["tableId"]."'");
                        if($check == 1):
                            $data["response"] = "success";
                            $data["responseType"] = "success";
                    ...
                    if(!empty($_POST["nameOfATable"])):
                        if(count($_POST["checkbox-select"]) > 0):
                            ...
                            foreach($_POST["checkbox-select"] as $selectedIds):
                                $databaseObj->select("tbl_property_type");
                                $databaseObj->where("`status` = '".$auth->visible()."' && `property_type_id` = '".$selectedIds."'");
                                $getData = $databaseObj->get();
                                //Checking If Data Is Available
                                if($getData != 0):
                                    foreach($getData as $rows):
                                ...
                                $tableData["property_type_log"] = json_encode($data_log);
                                $check = $databaseObj->update($_POST["nameOfATable"], $tableData, "`property_type_id` = '".$selectedIds."'");
                                $databaseObj->error();
                                if($check == 1):
                                    unset($tableData["property_type_log"]); 
                                    unset($tableData["status"]);
                ...
                if($authority == 1):
                    if(!empty($_POST["nameOfATable"])):
                        if(count($_POST["checkbox-select"]) > 0):
                            ...
                            //--------------------------------------------------------------------------------------------------
                            //Set Filename
                            $fileName = "Property-Type-".date("d-m-Y").".xlsx";
                            //Set BackGround
                            ...
                            $objPHPExcel->getProperties()->setCreator("IREMS")
                                                         ...
                                                         ->setSubject("Property Type")
                                                         ->setDescription("Property Type $fileName.")
                                                         ->setKeywords("$fileName")
                                                         ->setCategory("$fileName");
                            $objPHPExcel->getActiveSheet()->getColumnDimension('B')->setAutoSize(true);	
                            $objPHPExcel->getActiveSheet()->getColumnDimension('C')->setAutoSize(true);		
                            $objPHPExcel->setActiveSheetIndex(0)->mergeCells('B2:C2');
                            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B2', "Property Type");
                            cellColor('B2', '001F3F');
                            ...
                            // ----------------------------------------------------------------
                            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C4', "Property Type");
                                ...
                                if($getData != 0):
                                    foreach($getData as $rows):
                                        ...
                                        $objPHPExcel->setActiveSheetIndex(0)
                                            ...
                                            ->setCellValue('C'.$inc, $data_info->propertyType);
