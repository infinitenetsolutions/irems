<?php 
    ...
    $authority = 1;
    $buildingDir = "../../../assets/admin/building/";
            ...
            case "addData":
                if($authority == 1):
                    if(!empty($_POST["building"])):
                        ...
                        $tableData["building_info"] = json_encode($data_info);
                        $tableData["building_log"] = json_encode($data_log);
                        $tableData["status"] = $auth->visible();
                        $check = $databaseObj->insert("tbl_building", $tableData);
                    ...
                    if(!empty($_FILES["importedExcel"]["name"])):
                        if(move_uploaded_file($_FILES["importedExcel"]["tmp_name"] , $buildingDir.$randSix."_".$_FILES["importedExcel"]["name"])):
                            ...
                            foreach ($sheet_data as $row):   
                                if(!empty($row["A"])):
                                    $data_info = array(
                                                            "building"          =>      htmlspecialchars($row["A"], ENT_QUOTES)
                                                    );
                                    $data_log = array(
                                                        array(
                                                                "action"                =>      "imported",
                                    ...
                                    $check = $databaseObj->insert("tbl_building", $tableData);
                                    if($check == 1):
                                        unset($tableData["building_info"]); 
                                        unset($tableData["building_log"]); 
            ...
            case "editData":
                if($authority == 1):
                    if(!empty($_POST["editTableId"])):
                        ...
                        $databaseObj->where("`status` = '".$auth->visible()."' && `building_id` = '".$_POST["editTableId"]."'");
                        $getData = $databaseObj->get();
                        //Checking If Data Is Available
                        if($getData != 0):
                            $sno = 1;
                            foreach($getData as $rows):
                                $data_info = json_decode($rows["building_info"]);
                                $data_log = json_decode($rows["building_log"]);
                            endforeach;
                            if(!empty($_POST["editBuilding"])):
                                $edit_info = array(
                                                    "building"          =>      htmlspecialchars($_POST["editBuilding"], ENT_QUOTES)
                                                );
                                ...
                                $tableData["building_info"] = json_encode($edit_info);
                                $tableData["building_log"] = json_encode($data_log);
                                $tableData["status"] = $auth->visible();
                                $check = $databaseObj->update("tbl_building", $tableData, "`building_id` = '".$_POST["editTableId"]."'");
                                if($check == 1):
                                    $data["response"] = "success";
                                    $data["responseType"] = "success";
            ...
            case "deleteData":
                if($authority == 1):
                    if(!empty($_POST["tableName"] && $_POST["tableId"])):
                        $databaseObj->select("tbl_building");
                        $databaseObj->where("`status` = '".$auth->visible()."' && `building_id` = '".$_POST["tableId"]."'");
                        $getData = $databaseObj->get();
                        //Checking If Data Is Available
                        if($getData != 0):
                            $sno = 1;
                        ...
                        $check = $databaseObj->update($_POST["tableName"], $tableData, "`building_id` = '".$_POST["tableId"]."'");
                        if($check == 1):
                            $data["response"] = "success";
                            $data["responseType"] = "success";
                    ...
                    if(!empty($_POST["nameOfATable"])):
                        if(count($_POST["checkbox-select"]) > 0):
                            ...
                            foreach($_POST["checkbox-select"] as $selectedIds):
                                $databaseObj->select("tbl_building");
                                $databaseObj->where("`status` = '".$auth->visible()."' && `building_id` = '".$selectedIds."'");
                                $getData = $databaseObj->get();
                                //Checking If Data Is Available
                                if($getData != 0):
                                    foreach($getData as $rows):
                                ...
                                $tableData["building_log"] = json_encode($data_log);
                                $check = $databaseObj->update($_POST["nameOfATable"], $tableData, "`building_id` = '".$selectedIds."'");
                                $databaseObj->error();
                                if($check == 1):
                                    unset($tableData["building_log"]); 
                                    unset($tableData["status"]);
                                ...
                                if($getData != 0):
                                    foreach($getData as $rows):
                                        ...
                                        $objPHPExcel->setActiveSheetIndex(0)
                                            ...
                                            ->setCellValue('C'.$inc, $data_info->building);
