<?php
    ...
    if(isset($_POST["action"])):
        ...
        switch($_POST["action"]):
            ...
            case "addData":
                if($authority == 1):
                    if(!empty($_POST["departmentName"] && $_POST["designationName"] )):
                        $designation_info = array(
                                                "departmentName"           =>      htmlspecialchars($_POST["departmentName"], ENT_QUOTES),
                                                "designationName"           =>      htmlspecialchars($_POST["designationName"], ENT_QUOTES),
                                        );
                        ...
                        $tableData["manage_designation_info"] = json_encode($designation_info);
                        $tableData["manage_designation_log"] = json_encode($designation_log);
                        $tableData["status"] = $auth->visible();                 
                        $check = $databaseObj->insert("tbl_manage_designation", $tableData);
                        if($check == 1):
                    ...
                    if(!empty($_FILES["importedExcel"]["name"])):
                        if(move_uploaded_file($_FILES["importedExcel"]["tmp_name"] , $manageCategoryDir.$randSix."_".$_FILES["importedExcel"]["name"])):
                            ...
                            foreach ($sheet_data as $row):
                                if(!empty($row["A"])):
                                    $designation_info = array(
                                                            "departmentName"           =>      htmlspecialchars($row["A"], ENT_QUOTES),
                                                            "designationName"    =>      htmlspecialchars($row["B"], ENT_QUOTES)
                                                    );
                                    $designation_log = array(
                                                        array(
                                                                "action"                =>      "imported",
                                    ...
                                    $check = $databaseObj->insert("tbl_manage_designation", $tableData);
                                    if($check == 1):
                                        unset($tableData["manage_designation_info"]);
                                        unset($tableData["manage_designation_log"]);
            ...
            case "editData":
                if($authority == 1):
                    if(!empty($_POST["editTableId"])):
                        ...
                        $databaseObj->where("`status` = '".$auth->visible()."' && `manage_designation_id` = '".$_POST["editTableId"]."'");
                        $getData = $databaseObj->get();
                        //Checking If Data Is Available
                        if($getData != 0):
                            $sno = 1;
                            foreach($getData as $rows):
                                $designation_info = json_decode($rows["manage_designation_info"]);
                                $designation_log = json_decode($rows["manage_designation_log"]);
                            endforeach;
                            if(!empty($_POST["editdesignationName"] )):
                                $edit_info = array(
                                                        "departmentName"           =>      htmlspecialchars($_POST["editdepartmentName"], ENT_QUOTES),
                                                        "designationName"    =>      htmlspecialchars($_POST["editdesignationName"], ENT_QUOTES)
                                                );
                                ...
                                array_push($designation_log, $edit_log);
                                $tableData["manage_designation_info"] = json_encode($edit_info);
                                $tableData["manage_designation_log"] = json_encode($designation_log);
                                $tableData["status"] = $auth->visible();
                                ...
                                $databaseObj->where("`status` = '".$auth->visible()."'");
                                 ...
                                 $check = $databaseObj->update("tbl_manage_designation", $tableData, "`manage_designation_id` = '".$_POST["editTableId"]."'");
            ...
            case "deleteData":
                if($authority == 1):
                    if(!empty($_POST["tableName"] && $_POST["tableId"])):
                        $databaseObj->select("tbl_manage_designation");
                        $databaseObj->where("`status` = '".$auth->visible()."' && `manage_designation_id` = '".$_POST["tableId"]."'");
                        $getData = $databaseObj->get();
                        //Checking If Data Is Available
                        if($getData != 0):
                            $sno = 1;
                        ...
                        array_push($designation_log, $delete_log);
                        $tableData["manage_designation_log"] = json_encode($designation_log);
                        $tableData["status"] = $auth->trash();
                        $check = $databaseObj->update($_POST["tableName"], $tableData, "`manage_designation_id` = '".$_POST["tableId"]."'");
                        if($check == 1):
                            $data["response"] = "success";
                            $data["responseType"] = "success";
                    ...
                    if(!empty($_POST["nameOfATable"])):
                        if(count($_POST["checkbox-select"]) > 0):
                            ...
                            foreach($_POST["checkbox-select"] as $selectedIds):
                                $databaseObj->select("tbl_manage_designation");
                                $databaseObj->where("`status` = '".$auth->visible()."' && `manage_designation_id` = '".$selectedIds."'");
                                $getData = $databaseObj->get();
                                //Checking If Data Is Available
                                if($getData != 0):
                                    foreach($getData as $rows):
                                ...
                                array_push($designation_log, $delete_log);
                                $tableData["manage_designation_log"] = json_encode($designation_log);
                                $check = $databaseObj->update($_POST["nameOfATable"], $tableData, "`manage_designation_id` = '".$selectedIds."'");
                                $databaseObj->error();
                                if($check == 1):
                                    unset($tableData["manage_designation_log"]);
                                    unset($tableData["status"]);
                ...
                if($authority == 1):
                    if(!empty($_POST["nameOfATable"])):
                        if(count($_POST["checkbox-select"]) > 0):
                            ...
                            //--------------------------------------------------------------------------------------------------
                            //Set Filename
                            $fileName = "Designation-Management-".date("d-m-Y").".xlsx";
                            //Set BackGround
                            ...
                            $objPHPExcel->getProperties()->setCreator("IREMS")
                                                         ...
                                                         ->setTitle("Designation Management")
                                                         ->setSubject("Designation Management")
                                                         ->setDescription("Designation Management $fileName.")
                                                         ->setKeywords("$fileName")
                                                         ->setCategory("$fileName");
                            $objPHPExcel->getActiveSheet()->getColumnDimension('B')->setAutoSize(true);
                            $objPHPExcel->getActiveSheet()->getColumnDimension('C')->setAutoSize(true);
                            $objPHPExcel->setActiveSheetIndex(0)->mergeCells('B2:C2');
                            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B2', "Designation Management");
                            cellColor('B2', '001F3F');
                            ...
                            //Setting Header --------------------------------------------------
                            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B4', "Designation Name");
                            fontColor('B4', '001F3F', '10', 'serif', true);
                            $objPHPExcel->getActiveSheet()->getStyle('B4')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                            $objPHPExcel->getActiveSheet()->getStyle('B4')->applyFromArray($thinBorder);
                            // ----------------------------------------------------------------
                            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C4', "Designation Name");
                                ...
                                if($getData != 0):
                                    foreach($getData as $rows):
                                        ...
                                        $objPHPExcel->setActiveSheetIndex(0)
                                            ->setCellValue('B'.$inc, $department_info->departmentName)
                                            ->setCellValue('C'.$inc, $department_info->designationName);
