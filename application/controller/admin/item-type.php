<?php 
    ...
    $authority = 1;
    $buildingDir = "../../../assets/admin/item-type/";
            ...
            case "addData":
                if($authority == 1):
                    if(!empty($_POST["item-type"])):
                        $data_info = array(
                                                    "item_type"             =>      htmlspecialchars($_POST["item-type"], ENT_QUOTES),
                                                    "item_type_ab"          =>      htmlspecialchars($_POST["item-type-ab"], ENT_QUOTES)
                                        );
                        $data_log = array(
                                            array(
                                                    "action"                =>      "added",
                        ...
                        $tableData["item_type_info"] = json_encode($data_info);
                        $tableData["item_type_log"] = json_encode($data_log);
                    ...
                    if(!empty($_FILES["importedExcel"]["name"])):
                        if(move_uploaded_file($_FILES["importedExcel"]["tmp_name"] , $buildingDir.$randSix."_".$_FILES["importedExcel"]["name"])):
                            ...
                            foreach ($sheet_data as $row):   
                                if(!empty($row["A"])):
                                    $data_info = array(
                                                            "item_type_ab"          =>      htmlspecialchars($row["A"], ENT_QUOTES),
                                                            "item_type"             =>      htmlspecialchars($row["B"], ENT_QUOTES)
                                                    );
                                    $data_log = array(
                                                        array(
                                                                "action"                =>      "imported",
                                    ...
                                    $check = $databaseObj->insert("tbl_item_type", $tableData);
                                    if($check == 1):
                                        unset($tableData["item_type_info"]); 
                                        unset($tableData["item_type_log"]); 
            ...
            case "editData":
                if($authority == 1):
                    if(!empty($_POST["editTableId"])):
                        ...
                        $databaseObj->where("`status` = '".$auth->visible()."' && `item_type_id` = '".$_POST["editTableId"]."'");
                        $getData = $databaseObj->get();
                        //Checking If Data Is Available
                        if($getData != 0):
                            $sno = 1;
                            foreach($getData as $rows):
                                $data_info = json_decode($rows["item_type_info"]);
                                $data_log = json_decode($rows["item_type_log"]);
                            endforeach;
                            if(!empty($_POST["editItemType"])):
                                $edit_info = array(
                                                    "item_type"             =>      htmlspecialchars($_POST["editItemType"], ENT_QUOTES),
                                                    "item_type_ab"          =>      htmlspecialchars($_POST["editItemTypeAb"], ENT_QUOTES)
                                                );
                                ...
                                $tableData["item_type_info"] = json_encode($edit_info);
                                $tableData["item_type_log"] = json_encode($data_log);
                                $tableData["status"] = $auth->visible();
                                $check = $databaseObj->update("tbl_item_type", $tableData, "`item_type_id` = '".$_POST["editTableId"]."'");
                                if($check == 1):
                                    $data["response"] = "success";
                                    $data["responseType"] = "success";
            ...
            case "deleteData":
                if($authority == 1):
                    if(!empty($_POST["tableName"] && $_POST["tableId"])):
                        $databaseObj->select("tbl_item_type");
                        $databaseObj->where("`status` = '".$auth->visible()."' && `item_type_id` = '".$_POST["tableId"]."'");
                        $getData = $databaseObj->get();
                        //Checking If Data Is Available
                        if($getData != 0):
                            $sno = 1;
                        ...
                        $check = $databaseObj->update($_POST["tableName"], $tableData, "`item_type_id` = '".$_POST["tableId"]."'");
                        if($check == 1):
                            $data["response"] = "success";
                            $data["responseType"] = "success";
                    ...
                    if(!empty($_POST["nameOfATable"])):
                        if(count($_POST["checkbox-select"]) > 0):
                            ...
                            foreach($_POST["checkbox-select"] as $selectedIds):
                                $databaseObj->select("tbl_item_type");
                                $databaseObj->where("`status` = '".$auth->visible()."' && `item_type_id` = '".$selectedIds."'");
                                $getData = $databaseObj->get();
                                //Checking If Data Is Available
                                if($getData != 0):
                                    foreach($getData as $rows):
                                ...
                                $tableData["item_type_log"] = json_encode($data_log);
                                $check = $databaseObj->update($_POST["nameOfATable"], $tableData, "`item_type_id` = '".$selectedIds."'");
                                $databaseObj->error();
                                if($check == 1):
                                    unset($tableData["item_type_log"]); 
                                    unset($tableData["status"]);
                ...
                if($authority == 1):
                    if(!empty($_POST["nameOfATable"])):
                        if(count($_POST["checkbox-select"]) > 0):
                            ...
                            //--------------------------------------------------------------------------------------------------
                            //Set Filename
                            $fileName = "Items-".date("d-m-Y").".xlsx";
                            //Set BackGround
                            ...
                            $objPHPExcel->getProperties()->setCreator("IREMS")
                                                         ...
                                                         ->setTitle("")
                                                         ->setSubject("Items")
                                                         ->setDescription("Items $fileName.")
                                                         ->setKeywords("$fileName")
                                                         ->setCategory("$fileName");
                            $objPHPExcel->getActiveSheet()->getColumnDimension('B')->setAutoSize(true);	
                            $objPHPExcel->getActiveSheet()->getColumnDimension('C')->setAutoSize(true);		
                            $objPHPExcel->setActiveSheetIndex(0)->mergeCells('B2:C2');
                            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B2', "Items");
                            cellColor('B2', '001F3F');
                            ...
                            // ----------------------------------------------------------------
                            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C4', "Items");
                                ...
                                if($getData != 0):
                                    foreach($getData as $rows):
                                        ...
                                        $objPHPExcel->setActiveSheetIndex(0)
                                            ...
                                            ->setCellValue('C'.$inc, $data_info->item_type);
