<?php 
    ...
    if(isset($_POST["action"])):
        ...
        switch($_POST["action"]):
            ...
            case "addData":
                if($authority == 1):
                    if(!empty($_POST["projectName"] && $_POST["phase"] && $_POST["building"] && $_POST["floors"] && $_POST["flat_no"] && $_POST["propertyPriceDeal"] && $_POST["firstApplicantName"] && $_POST["firstApplicantParentOf"] && $_POST["firstApplicantPhoneNumber"] && $_POST["firstApplicantUsername"] && $_POST["logPass"] && $_POST["firstApplicantEmailId"] && $_POST["firstApplicantDateOfBirth"] && $_POST["firstApplicanPanNumber"] && $_POST["firstApplicanAadharNumber"] && ($_POST["firstApplicanPermanentAddress"] || $_POST["firstApplicanCorrespondenceAddress"]))):
                        $flag = 1;
                        $downPayment = 0;
                        $customerStatus = "";
                        ...
                        if(!empty($_POST["paymentAmount"])):
                            $data_payment_info = array(
                                                    array(
                                                        ...
                                                        "paymentMode"               =>      htmlspecialchars($_POST["paymentAmountMode"], ENT_QUOTES),
                                                        "paymentBankName"           =>      htmlspecialchars($_POST["paymentAmountBankName"], ENT_QUOTES),
                            ...
                            if(empty($_POST["paymentStuctureDate"][$downPayment])):
                                $flag = 0;
                            else:
                                $paymentStuctureDate = $_POST["paymentStuctureDate"][$downPayment];
                            endif;
                            $tempPaymentStucture = array(
                                                    ...
                                                    "paymentStuctureDate"                       =>      htmlspecialchars($paymentStuctureDate, ENT_QUOTES),
                                                    "paymentStucturePaid"                       =>      htmlspecialchars($_POST["paymentAmount"], ENT_QUOTES),
                                                    "paymentStucturePaidRemark"                 =>      "Booking Amount",
                                                    "paymentStucturePaidDate"                   =>      htmlspecialchars($paymentStuctureDate, ENT_QUOTES),
                        ...
                        for($i = $downPayment; $i<$_POST["totalNumberOfDivision"]; $i++):
                            ...
                            if(empty($_POST["paymentStuctureDate"][$i])):
                                $flag = 0;
                            else:
                                $paymentStuctureDate = $_POST["paymentStuctureDate"][$i];
                            endif;
                            $tempPaymentStucture = array(
                                                    "paymentStuctureCompletion"                 =>      htmlspecialchars($paymentStucturePercentageOfAmount, ENT_QUOTES),
                                                    "paymentStuctureAmount"                     =>      htmlspecialchars($paymentStuctureAmount, ENT_QUOTES),
                        ...
                        if($flag == 1):
                            ...
                            $data_log = array(
                                                    ...
                                                    "pass"                          =>      htmlspecialchars($logPass, ENT_QUOTES),
                                                    "oldPass"                       =>      "",
                                                    "auth"                          =>      "",
                            ...
                            foreach($getCategoryData as $categoryrows):
                                ...
                                if(!empty($customer_info->firstApplicantUsername)){
                            ...
                            if($check == 1):
                                $customer_id = $databaseObj->last_inserted_id();
                                //*********************************************************************************************************
                                //sEND mESSAGE tO tHE cUSTOMER aND aLSO tO tHE sECOND aPPLICANT aND aDMIN tOO *****************************
                                //*********************************************************************************************************
                                $name = $_POST["firstApplicantName"];
                                $username = $_POST["firstApplicantUsername"];
                                $password = $_POST["logPass"];
                                $message="Dear $name, Your Form has been Successfully submitted. \nClick the link below to view your details .\nhttp://https://srinathhomes.in/irems/customer/\nUsername: $username \nPassword: $password \n\nRegards,\nSrinath Homes";
                                $databaseObj->send_to_phone($_POST["firstApplicantPhoneNumber"], $message);
                                //*********************************************************************************************************
                                //sEND mESSAGE tO tHE cUSTOMER aND aLSO tO tHE sECOND aPPLICANT aND aDMIN tOO *****************************
                                //*********************************************************************************************************

                                $databaseObj->select("tbl_properties");
                                $databaseObj->where("`status` = '".$auth->visible()."' && `projects_id` = '".$_POST["projectName"]."'");
                                ...
                                if($getData != 0):
                                    ...
                                    foreach($getData as $rows):
                                        ...
                                        if($properties_info->phase == $_POST["phase"] && $properties_info->building == $_POST["building"]):
                                            // $floor = $properties_info->floors[($_POST["floors"] - 1)];
                                            $checkFloors = 1;
                                            foreach($properties_info->floors as $allFloors):
                                                if($checkFloors == $_POST["floors"]):
                                                    foreach($allFloors as $allFlats):
                                                        if($allFlats->flat_no == $_POST["flat_no"]):
                                                            $allFlats->status = "reserved";
                                                            $allFlats->customer_details->customer_id = $customer_id;
                                                            break;
                                                        endif;
                                                    endforeach;
                                                    break;
                                            ...
                                            endforeach;
                                            $tableData = array();
                                            $tableData["properties_info"] = json_encode($properties_info);
                                            $databaseObj->data = array();
                                            $databaseObj->dataVal = "";
                                            $databaseObj->sql = "";
                                            $databaseObj->update("tbl_properties", $tableData, "`properties_id` = '".$rows["properties_id"]."'");
                                            echo $databaseObj->error();
                                            break;
                                        endif;
                                    endforeach;
                    ...
                    if(!empty($_POST["editTableId"])):
                        ...
                        for($i = 0; $i<$_POST["editTotalNumberOfDivision"]; $i++):
                            ...
                            if(isset($_POST["paymentStuctureStatus"][$i])):
                                $paymentStucturePaid = (!empty($_POST["paymentStucturePaid"][$i])?$_POST["paymentStucturePaid"][$i]:'');
                                $paymentStucturePaidRemark = (!empty($_POST["paymentStucturePaidRemark"][$i])?$_POST["paymentStucturePaidRemark"][$i]:'');
                            ...
                            $paidAmt=($paymentStuctureStatus?htmlspecialchars($paymentStuctureAmount, ENT_QUOTES):'');
                            $tempPaymentStucture = array(
                                                    "paymentStuctureCompletion"                 =>      htmlspecialchars($paymentStucturePercentageOfAmount, ENT_QUOTES),
                                                    "paymentStuctureAmount"                     =>      htmlspecialchars($paymentStuctureAmount, ENT_QUOTES),
                                                    "paymentStuctureRemark"                     =>      htmlspecialchars($paymentStuctureRemark, ENT_QUOTES),
                                                    "paymentStucturePaid"                       =>      $paidAmt,
                ...
                if($authority == 1):
                    if(!empty($_POST["editTableId"])):
                        ...
                        if($getData != 0):
                            $sno = 1;
                            foreach($getData as $rows):
                                $extra_info = json_decode($rows["customer_extra_payment_structure"]);
                               
                             
                                $property_info = json_decode($rows["customer_property_info"]);
                            ...
                            if(!empty($_POST["editfirstApplicantTitle"]  && $_POST["editfirstApplicantName"] && $_POST["editphase"] && $_POST["editbuilding"]  && $_POST["editfloors"] && $_POST["editflat_no"]   && $_POST["editfirstApplicantParentOf"] && $_POST["editfirstApplicantPhoneNumber"] && $_POST["editfirstApplicantEmailId"] && $_POST["editfirstApplicantDateOfBirth"] && $_POST["editfirstApplicantAge"] && $_POST["editfirstApplicantReligion"] && $_POST["editfirstApplicanPanNumber"] &&  $_POST["editfirstApplicanAadharNumber"] &&  ($_POST["editfirstApplicanPermanentAddress"]))):
                                $data_property_info = array(
                                                "projectName"               =>      $_POST["editprojectName"],
                                                "phase"                     =>      $_POST["editphase"],
                                                "building"                  =>      $_POST["editbuilding"],
                                                "floors"                    =>      $_POST["editfloors"],
                                                "flat_no"                   =>      $_POST["editflat_no"],
                                                "propertyPriceDeal"         =>      htmlspecialchars($_POST["editpropertyPriceDeal"], ENT_QUOTES),
                                                "propertyCarParkings"       =>      htmlspecialchars($_POST["editpropertyCarParkings"], ENT_QUOTES),
                                                "CarParkingAmount"          =>      htmlspecialchars($_POST["editCarParkingAmount"], ENT_QUOTES),
                                                "propertyScooterParkings"   =>      htmlspecialchars($_POST["editpropertyScooterParkings"], ENT_QUOTES),
                                                "ScooterParkingAmount"      =>      htmlspecialchars($_POST["editScooterParkingAmount"], ENT_QUOTES)
                                              );
                                $slashNandR =  array("\n", "\r");
                                         $editextratempProperty = array();
                                         for($i = 1; $i <= count($_POST["editExtraAmount"]); $i++):
                                            $temp_item = array(
                                                                "ExtraAmount" => htmlspecialchars(str_replace($slashNandR, "", $_POST["editExtraAmount"][$i]), ENT_QUOTES),
                                                                "ExtraAmountRemarks" => htmlspecialchars(str_replace($slashNandR, "", $_POST["editExtraAmountRemarks"][$i]), ENT_QUOTES)
                                                                
                                                                

                                                        );
                                            array_push($editextratempProperty, $temp_item);
                                         endfor;
                                         echo $_POST["editPaymentTotalNumberOfDivision"];
                                          $paymentStucture = array();
                                        for($i = 1; $i<= $_POST["editPaymentTotalNumberOfDivision"]; $i++):
                                        
                                         
                                        
                                       $tempPaymentStucture = array(
                                                    "paymentStuctureCompletion"                 =>      htmlspecialchars($_POST["editpaymentStuctureCompletion$i"], ENT_QUOTES),
                                                    "paymentStuctureAmount"                     =>      htmlspecialchars($_POST["editpaymentStuctureAmount$i"], ENT_QUOTES),
                                                    "paymentStuctureRemark"                     =>      htmlspecialchars($_POST["editpaymentStuctureRemark$i"], ENT_QUOTES),
                                                     "paymentStuctureDate"                       =>      htmlspecialchars($_POST["editpaymentStuctureDate$i"], ENT_QUOTES)
                                ...
                                $tableData["customer_second_applicant"] = json_encode($second_edit_info);
                                  $tableData["customer_extra_payment_structure"] = json_encode($editextratempProperty);
                                 $tableData["customer_property_info"] =   json_encode($data_property_info);
                                    ...
                                    foreach($properties_info->floors as $allFloors):
                                        if($checkFloors == $_POST["floors"]):
                                            foreach($allFloors as $allFlats):
                                                if($allFlats->flat_no == $_POST["flat_no"]):
                                                    $allFlats->status = "";
                                                    $allFlats->customer_details->customer_id = "";
                                                    break;
                                                endif;
                                            endforeach;
                                            break;
