<?php 
    ...
    $authority = 1;
    $accommodationTypeDir = "../../../assets/admin/accommodation-type/";
    if(isset($_POST["action"])):
        ...
        switch($_POST["action"]):
            ...
            case "addData":
                if($authority == 1):
                    if(!empty($_POST["accommodationType"])):
                        $data_info = array(
                                                    "accommodationType"          =>      htmlspecialchars($_POST["accommodationType"], ENT_QUOTES)
                                        );
                        $data_log = array(
                                            array(
                                                    "action"                =>      "added",
                        ...
                        $tableData["accommodation_type_info"] = json_encode($data_info);
                        $tableData["accommodation_type_log"] = json_encode($data_log);
                        $tableData["status"] = $auth->visible();
                        $check = $databaseObj->insert("tbl_accommodation_type", $tableData);
            ...
            case "importData":
                if($authority == 1):
                    if(!empty($_FILES["importedExcel"]["name"])):
                        if(move_uploaded_file($_FILES["importedExcel"]["tmp_name"] , $accommodationTypeDir.$randSix."_".$_FILES["importedExcel"]["name"])):
                            $fileName = $randSix."_".$_FILES["importedExcel"]["name"];
                            $file_type	= PHPExcel_IOFactory::identify($accommodationTypeDir.$fileName);
                            $objReader	= PHPExcel_IOFactory::createReader($file_type);
                            $objPHPExcel = $objReader->load($accommodationTypeDir.$fileName);
                            $sheet_data	= $objPHPExcel->getActiveSheet()->toArray(null,true,true,true);
                            $sno = 0;
                            foreach ($sheet_data as $row):   
                                if(!empty($row["A"])):
                                    $data_info = array(
                                                            "accommodationType"          =>      htmlspecialchars($row["A"], ENT_QUOTES)
                                                    );
                                    $data_log = array(
                                                        array(
                                                                "action"                =>      "imported",
                                    ...
                                    $check = $databaseObj->insert("tbl_accommodation_type", $tableData);
                                    if($check == 1):
                                        unset($tableData["accommodation_type_info"]); 
                                        unset($tableData["accommodation_type_log"]); 
                            ...
                            endforeach;
                            if($sno > 0):
                                unlink($accommodationTypeDir.$fileName);
                                $data["response"] = "success";
                                $data["responseType"] = "success";
            ...
            case "editData":
                if($authority == 1):
                    if(!empty($_POST["editTableId"])):
                        ...
                        $databaseObj->where("`status` = '".$auth->visible()."' && `accommodation_type_id` = '".$_POST["editTableId"]."'");
                        $getData = $databaseObj->get();
                        //Checking If Data Is Available
                        if($getData != 0):
                            $sno = 1;
                            foreach($getData as $rows):
                                $data_info = json_decode($rows["accommodation_type_info"]);
                                $data_log = json_decode($rows["accommodation_type_log"]);
                            endforeach;
                            if(!empty($_POST["editAccommodationType"])):
                                $edit_info = array(
                                                    "accommodationType"          =>      htmlspecialchars($_POST["editAccommodationType"], ENT_QUOTES)
                                                );
                                ...
                                $tableData["accommodation_type_info"] = json_encode($edit_info);
                                $tableData["accommodation_type_log"] = json_encode($data_log);
                                $tableData["status"] = $auth->visible();
                                $check = $databaseObj->update("tbl_accommodation_type", $tableData, "`accommodation_type_id` = '".$_POST["editTableId"]."'");
                                if($check == 1):
                                    $data["response"] = "success";
                                    $data["responseType"] = "success";
            ...
            case "deleteData":
                if($authority == 1):
                    if(!empty($_POST["tableName"] && $_POST["tableId"])):
                        $databaseObj->select("tbl_accommodation_type");
                        $databaseObj->where("`status` = '".$auth->visible()."' && `accommodation_type_id` = '".$_POST["tableId"]."'");
                        $getData = $databaseObj->get();
                        //Checking If Data Is Available
                        if($getData != 0):
                            $sno = 1;
                        ...
                        $check = $databaseObj->update($_POST["tableName"], $tableData, "`accommodation_type_id` = '".$_POST["tableId"]."'");
                        if($check == 1):
                            $data["response"] = "success";
                            $data["responseType"] = "success";
                    ...
                    if(!empty($_POST["nameOfATable"])):
                        if(count($_POST["checkbox-select"]) > 0):
                            ...
                            foreach($_POST["checkbox-select"] as $selectedIds):
                                $databaseObj->select("tbl_accommodation_type");
                                $databaseObj->where("`status` = '".$auth->visible()."' && `accommodation_type_id` = '".$selectedIds."'");
                                $getData = $databaseObj->get();
                                //Checking If Data Is Available
                                if($getData != 0):
                                    foreach($getData as $rows):
                                ...
                                $tableData["accommodation_type_log"] = json_encode($data_log);
                                $check = $databaseObj->update($_POST["nameOfATable"], $tableData, "`accommodation_type_id` = '".$selectedIds."'");
                                $databaseObj->error();
                                if($check == 1):
                                    unset($tableData["accommodation_type_log"]); 
                                    unset($tableData["status"]);
                ...
                if($authority == 1):
                    if(!empty($_POST["nameOfATable"])):
                        if(count($_POST["checkbox-select"]) > 0):
                            ...
                            //--------------------------------------------------------------------------------------------------
                            //Set Filename
                            $fileName = "Accommodation-Type-".date("d-m-Y").".xlsx";
                            //Set BackGround
                            ...
                            $objPHPExcel->getProperties()->setCreator("IREMS")
                                                         ...
                                                         ->setTitle("Accommodation Type")
                                                         ->setSubject("Accommodation Type")
                                                         ->setDescription("Accommodation Type $fileName.")
                                                         ->setKeywords("$fileName")
                                                         ->setCategory("$fileName");
                            $objPHPExcel->getActiveSheet()->getColumnDimension('B')->setAutoSize(true);	
                            $objPHPExcel->getActiveSheet()->getColumnDimension('C')->setAutoSize(true);		
                            $objPHPExcel->setActiveSheetIndex(0)->mergeCells('B2:C2');
                            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B2', "Accommodation Type");
                            cellColor('B2', '001F3F');
                            ...
                            // ----------------------------------------------------------------
                            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C4', "Accommodation Type");
                                ...
                                if($getData != 0):
                                    foreach($getData as $rows):
                                        ...
                                        $objPHPExcel->setActiveSheetIndex(0)
                                            ...
                                            ->setCellValue('C'.$inc, $data_info->accommodationType);
