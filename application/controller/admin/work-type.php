<?php 
    ...
    $authority = 1;
    $buildingDir = "../../../assets/admin/work-type/";
            ...
            case "addData":
                if($authority == 1):
                    if(!empty($_POST["work-type"])):
                        $data_info = array(
                                                    "work_type"          =>      htmlspecialchars($_POST["work-type"], ENT_QUOTES)
                                        );
                        $data_log = array(
                                            array(
                                                    "action"                =>      "added",
                        ...
                        $tableData["work_type_info"] = json_encode($data_info);
                        $tableData["work_type_log"] = json_encode($data_log);
                    ...
                    if(!empty($_FILES["importedExcel"]["name"])):
                        if(move_uploaded_file($_FILES["importedExcel"]["tmp_name"] , $buildingDir.$randSix."_".$_FILES["importedExcel"]["name"])):
                            ...
                            foreach ($sheet_data as $row):   
                                if(!empty($row["A"])):
                                    $data_info = array(
                                                            "work_type"          =>      htmlspecialchars($row["A"], ENT_QUOTES)
                                                    );
                                    $data_log = array(
                                                        array(
                                                                "action"                =>      "imported",
                                    ...
                                    $check = $databaseObj->insert("tbl_work_type", $tableData);
                                    if($check == 1):
                                        unset($tableData["work_type_info"]); 
                                        unset($tableData["work_type_log"]); 
            ...
            case "editData":
                if($authority == 1):
                    if(!empty($_POST["editTableId"])):
                        ...
                        $databaseObj->where("`status` = '".$auth->visible()."' && `work_type_id` = '".$_POST["editTableId"]."'");
                        $getData = $databaseObj->get();
                        //Checking If Data Is Available
                        if($getData != 0):
                            $sno = 1;
                            foreach($getData as $rows):
                                $data_info = json_decode($rows["work_type_info"]);
                                $data_log = json_decode($rows["work_type_log"]);
                            endforeach;
                            if(!empty($_POST["editWorkType"])):
                                $edit_info = array(
                                                    "work_type"          =>      htmlspecialchars($_POST["editWorkType"], ENT_QUOTES)
                                                );
                                ...
                                $tableData["work_type_info"] = json_encode($edit_info);
                                $tableData["work_type_log"] = json_encode($data_log);
                                $tableData["status"] = $auth->visible();
                                $check = $databaseObj->update("tbl_work_type", $tableData, "`work_type_id` = '".$_POST["editTableId"]."'");
                                if($check == 1):
                                    $data["response"] = "success";
                                    $data["responseType"] = "success";
            ...
            case "deleteData":
                if($authority == 1):
                    if(!empty($_POST["tableName"] && $_POST["tableId"])):
                        $databaseObj->select("tbl_work_type");
                        $databaseObj->where("`status` = '".$auth->visible()."' && `work_type_id` = '".$_POST["tableId"]."'");
                        $getData = $databaseObj->get();
                        //Checking If Data Is Available
                        if($getData != 0):
                            $sno = 1;
                        ...
                        $check = $databaseObj->update($_POST["tableName"], $tableData, "`work_type_id` = '".$_POST["tableId"]."'");
                        if($check == 1):
                            $data["response"] = "success";
                            $data["responseType"] = "success";
                    ...
                    if(!empty($_POST["nameOfATable"])):
                        if(count($_POST["checkbox-select"]) > 0):
                            ...
                            foreach($_POST["checkbox-select"] as $selectedIds):
                                $databaseObj->select("tbl_work_type");
                                $databaseObj->where("`status` = '".$auth->visible()."' && `work_type_id` = '".$selectedIds."'");
                                $getData = $databaseObj->get();
                                //Checking If Data Is Available
                                if($getData != 0):
                                    foreach($getData as $rows):
                                ...
                                $tableData["work_type_log"] = json_encode($data_log);
                                $check = $databaseObj->update($_POST["nameOfATable"], $tableData, "`work_type_id` = '".$selectedIds."'");
                                $databaseObj->error();
                                if($check == 1):
                                    unset($tableData["work_type_log"]); 
                                    unset($tableData["status"]);
                    ...
                    if(!empty($_POST["nameOfATable"])):
                        if(count($_POST["checkbox-select"]) > 0):
                            ...
                            $objPHPExcel->getProperties()->setCreator("IREMS")
                                                         ...
                                                         ->setTitle("")
                                                         ->setSubject("Work")
                                                         ->setDescription("Work $fileName.")
                                                         ->setKeywords("$fileName")
                                                         ->setCategory("$fileName");
                            $objPHPExcel->getActiveSheet()->getColumnDimension('B')->setAutoSize(true);	
                            $objPHPExcel->getActiveSheet()->getColumnDimension('C')->setAutoSize(true);		
                            $objPHPExcel->setActiveSheetIndex(0)->mergeCells('B2:C2');
                            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B2', "Work");
                            cellColor('B2', '001F3F');
                            ...
                            // ----------------------------------------------------------------
                            $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C4', "Work");
                                ...
                                if($getData != 0):
                                    foreach($getData as $rows):
                                        ...
                                        $objPHPExcel->setActiveSheetIndex(0)
                                            ...
                                            ->setCellValue('C'.$inc, $data_info->work_type);
