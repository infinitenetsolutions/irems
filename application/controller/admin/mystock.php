            ...
            case "editData":
                ...
                if($authority == 1):
                    if(!empty($_POST["editTableId"])):
                        ...
                        $databaseObj->where("`status` = '".$auth->visible()."' && `itemCode` = '".$_POST["editItemCode"]."'");
                        ...
                        if($getData != 0):
                            ...
                            if(!empty($_POST["editItemName"])):
                                            ...
                                            $Qty                =      htmlspecialchars($_POST["editQty"], ENT_QUOTES);
                ...
                if($authority == 1):
                    if(!empty($_POST["tableName"] && $_POST["tableId"])):
                        ...
                        $databaseObj->where("`status` = '".$auth->visible()."' && `manage_stock_id` = '".$_POST["tableId"]."'");
                         ...
                         // $databaseObj->where("`status` = '".$auth->visible()."' && `project` = '".$_POST["project"]."' && `itemCode` = '".$_POST["ItemCode"]."'&& `itemName` = '".$_POST["itemName"]."'");
                        ...
                        if($getData != 0):
                            ...
                            foreach($getData as $rows):
                                $stock_log = json_decode($rows["manage_stock_log"]);
                            endforeach;
                        endif;
                        $delete_log = array(
                                                "action"                =>      "deleted",
                        ...
                        // array_push($stock_log, $delete_log);
                ...
                if($authority == 1):
                    ...
                    // echo "</pre>";
                     ...
                     endforeach;
                        ...
                        // $commit_edit = json_decode($rowsprojects["commit_edit"]);
                        // $commit_delete = json_decode($rowsprojects["commit_delete"]);
                     
                    
                      $databaseObj->select("tbl_projects");
                      // $databaseObj->where("`status` = '".$auth->visible()."'" );
                      $databaseObj->where("`status` = '".$auth->visible()."' && `projects_id` = '".$rowsmanage['project']."'");
                      $databaseObj->order_by("`projects_id` DESC");
                     $getProjectData = $databaseObj->get();
                    //     echo "<pre>";
                    // print_r($getProjectData);
                    // echo "</pre>"; 
                    $project_id=$getProjectData['0']['projects_id'];                 
                    //Checking If Data Is Available
                    if($getProjectData != 0):
                     $sno = 1;
                     foreach($getProjectData as $rowsprojects):
                        $projects_info = json_decode($rowsprojects["projects_info"]);
                        $commit_edit = json_decode($rowsprojects["commit_edit"]);
                        $commit_delete = json_decode($rowsprojects["commit_delete"]);
                     endforeach;
                        ...
                        // array_push($stock_log, $delete_log);
                        $tableData["manage_stock_log"] = json_encode($commit_log);
                        $tableprojectData["projects_log"] = json_encode($commit_log);
                        $tableprojectData["commit_edit"] = "1";
                        $tableprojectData["commit_delete"] = "1";
                        $check = $databaseObj->update("tbl_projects", $tableprojectData, "`projects_id` = '".$project_id."' && `status` = '".$auth->visible()."'");
                    //       echo "<pre>";
                    //       print_r($check);
                    // echo "</pre>"; 
                    // exit(); 
                        // $check = $databaseObj->update($_POST["tableName"], $tableData, "`project` = '".$_POST["project"]."' && `itemCode` = '".$_POST["ItemCode"]."'");
                        if($check == 1):
                            $data["response"] = "success";
                            $data["responseType"] = "success";
                            $data["responseMessage"] = "Data updated successfully!!!";
                        else:
                            $data["response"] = "error";
                            $data["responseType"] = "error";
                            $data["responseMessage"] = "Something went wrong please try again!!!";
                ...
                if($authority == 1):
                    if(!empty($_POST["nameOfATable"])):
                        if(count($_POST["checkbox-select"]) > 0):
                            ...
                            foreach($_POST["checkbox-select"] as $selectedIds):
                                $databaseObj->select("tbl_manage_stock");
                                $databaseObj->where("`status` = '".$auth->visible()."' && `manage_stock_id` = '".$selectedIds."'");
                                $getData = $databaseObj->get();
                                //Checking If Data Is Available
                                if($getData != 0):
                                    foreach($getData as $rows):
                                ...
                                array_push($stock_log, $delete_log);
                                $tableData["manage_stock_log"] = json_encode($stock_log);
                                $tableData["status"] = $auth->trash();
                                $check = $databaseObj->update($_POST["nameOfATable"], $tableData, "`manage_stock_id` = '".$selectedIds."'");
                                $databaseObj->error();
                                if($check == 1):
                                    unset($tableData["manage_stock_log"]);
                                    unset($tableData["status"]);
                                    $databaseObj->sql = "";
                                    $databaseObj->data = array();
                                    $databaseObj->dataVal = "";
