                    ...
                    if(!empty($_POST["editTableId"])):
                        ...
                        if($getData != 0):
                            ...
                            endforeach;
                                ...
                                $edit_info = array(
                "supplierName"              =>      (empty($supplier_info->supplierName)?"":$supplier_info->supplierName),
                "supplierCity"              =>      (empty($supplier_info->supplierCity)?"":$supplier_info->supplierCity),
                "supplierState"             =>      (empty($supplier_info->supplierState)?"":$supplier_info->supplierState),
                "supplierCountry"           =>      (empty($supplier_info->supplierCountry)?"":$supplier_info->supplierCountry),
                "supplierAddress"           =>      (empty($supplier_info->supplierAddress)?"":$supplier_info->supplierAddress),
                "supplierConcernPersonName" =>      (empty($supplier_info->supplierConcernPersonName)?"":$supplier_info->supplierConcernPersonName),
                "supplierOfficePhone"       =>      (empty($supplier_info->supplierOfficePhone)?"":$supplier_info->supplierOfficePhone),
                "supplierPhone"             =>      (empty($supplier_info->supplierPhone)?"":$supplier_info->supplierPhone),
                "supplierAlternatePhone"    =>      (empty($supplier_info->supplierAlternatePhone)?"":$supplier_info->supplierAlternatePhone),
                "supplierEmail"             =>      (empty($supplier_info->supplierEmail)?"":$supplier_info->supplierEmail),
                "supplierGstin"             =>      (empty($supplier_info->supplierGstin)?"":$supplier_info->supplierGstin),
                "approval"                  =>      "Approved"
                                                );
                                ...
                                if($check == 1):
                                    $data["response"] = "success";
                                    $data["responseType"] = "success";
                                    $data["responseMessage"] = "Vendor Approved successfully!!!";
                                else:
                                    $data["response"] = "error";
                                    $data["responseType"] = "error";
                                    $data["responseMessage"] = "Something went wrong please try again!!!";
