<?php 
    ...
    if(isset($_POST["action"])){
                        ...
                        if(!empty($logIp && $logDate && $logTime)){
                            ...
                            if($getData != 0){
                                foreach($getData as $rows){
									...
									$data["redirect"] = "customer/".$_COOKIE["lastInfoCustomer"];
									else
									$data["redirect"] = "customer/dashboard";
									} else{
									$data["error"] = "error";
									$data["errorType"] = "error";
									$data["message"] = "Something went wrong, Please try again later!!!";
        ...
        elseif($_POST["action"] == "lockAppCustomer"){
            //Unset the logInfo SESSION
            if(isset($_SESSION["logInfo"])){
                unset($_SESSION["logInfo"]);
                if(!isset($_SESSION["logInfo"]))
        ...
        elseif($_POST["action"] == "logOutAppCustomer"){
            $logInfo = json_decode($_COOKIE["logInfo"]);
            //Get Data From Admin Table
            $databaseObj->select("tbl_customer");
            $databaseObj->where("`status` = '".md5($databaseObj->visible)."'");
