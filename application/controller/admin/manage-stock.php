           ...
           case "editData":
                ...
                if($authority == 1):
                    ...
                    // Get existing data
                        ...
                        // Get Count Data
                        $databaseObj->select_count_row("tbl_manage_stock","manage_stock_id");
                        $databaseObj->where("`status` = '".$auth->visible()."' && `project` = '".$_POST["project"]."' && `itemCode` = '".$_POST["ItemCode"]."'");
                        $getData = $databaseObj->get();

                                   
                        //Checking If Data Is Available
                        if($getData['0']['countnum'] == 0)
                        {
                           if(!empty($_POST["ItemName"]))
                           {
                                            ...
                                            $edit_log = array(
                                                ...
                                                $check = $databaseObj->insert("tbl_manage_stock",$tableData);
                               ...
                               if(!empty($_POST["ItemName"]))
                               {
                                            ...
                                            $edit_log = array(
                                                ...
                                                $tableData["status"] = $auth->visible();  
                                                 ...
                                                 //   endforeach;
                                                 //  endif; 
                                                 $check = $databaseObj->update("tbl_manage_stock", $tableData, "`project` = '".$_POST["project"]."' && `itemCode` = '".$_POST["ItemCode"]."'");
                                                 // echo "<pre>";
                                                 //  print_r($check);
                                                 //  echo "</pre>";
                                                  // exit(0);  
                                                  // $check = $databaseObj->update_table("tbl_manage_stock", $tableData);
                                                // $check = $databaseObj->insert("tbl_manage_stock",$tableData);
